<template>
  <div>
    <NuxtLayout name="default">
      <template #title>{{ $t("Liste des bouquineries") }}</template>
      <template #inner-title>{{ $t("Liste des bouquineries") }}</template>
      {{
        $t(
...
const fetchBookstores = async () => {
  bookstores = (await useFetch("/bookstoreComment/list")).data.value
    .map((bookstore) => {
