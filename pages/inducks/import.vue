<template>
  <div>
    <NuxtLayout name="default">
      <template #title>{{ $t("Importer ma collection Inducks") }}</template>
      <template #inner-title>{{
        $t("Importer ma collection Inducks")
      }}</template>
      ...
      <form v-else-if="step === 1" id="inducks-import" method="post" action="">
        <b-alert show variant="info">
          ...
          <ol>
            ...
            <li>
              {{ $t("Collez ce texte dans la partie droite de la page.") }}
            </li>
            <li>
      ...
      <template v-else-if="step === 2">
        ...
        <b-alert
          v-if="
            issuesNotReferenced?.length || issuesAlreadyInCollection?.length
          "
          show
          variant="warning"
        >
          ...
          <template v-if="issuesNotReferenced?.length">
            ...
            <Accordion
              ...
              @bv::toggle::collapse="
                expandedNotImportableAccordion = 'not-found'
              "
            >
              <template #header>
            ...
            >
              <template #first>
                <b-form-select-option :value="null" disabled>
                  {{
                    $t(
                      "Choisissez un état par défaut pour les nouveaux numéros"
                    )
                  }}
...
const importIssues = async () => {
  ...
  for (const publicationCode in importableIssuesByPublicationCode) {
    if (
      ...
      await useFetch("/api/collection/issues", {
        method: "POST",
        data: {
          publicationCode,
