...
create table abonnements_sorties (
    ...
    issuecode varchar(25) default concat(
        convert(`Pays` using utf8mb3),
        '/',
        convert(`Magazine` using utf8mb3),
        ' ',
        convert(`Numero` using utf8mb3)
    ) not null,
...
create table achats (
    ...
    constraint user_date_description_unique unique (ID_User, Date, Description)
...
create table auteurs_pseudos (
    ...
    constraint auteurs_pseudos_uindex unique (ID_user, NomAuteurAbrege)
) engine = MyISAM collate = utf8mb3_bin row_format = DYNAMIC;
...
create table bibliotheque_ordre_magazines (
    ...
    constraint bibliotheque_ordre_magazines_uindex unique (ID_Utilisateur, publicationcode)
) engine = MyISAM collate = latin1_german2_ci;
...
create table numeros (
    ...
    publicationcode varchar(14) default concat(
        convert(`Pays` using utf8mb3),
        '/',
        convert(`Magazine` using utf8mb3)
    ) not null,
    ...
    short_issuecode varchar(25) collate utf8mb3_unicode_ci as (
        concat(
            convert(`Pays` using utf8mb3),
            '/',
            convert(`Magazine` using utf8mb3),
    ...
    issuecode varchar(25) collate utf8mb3_unicode_ci null
) engine = MyISAM collate = latin1_german2_ci;
create index Numero_Utilisateur on numeros (Pays, Magazine, Numero, ID_Utilisateur);
create index Numero_nospace_Utilisateur on numeros (Pays, Magazine, Numero_nospace, ID_Utilisateur);
create index Pays_Magazine_Numero on numeros (Pays, Magazine, Numero);
create index Pays_Magazine_Numero_DateAjout on numeros (DateAjout, Pays, Magazine, Numero);
create index Utilisateur on numeros (ID_Utilisateur);
create index issuecode_creationdate on numeros (DateAjout);
create index issuecode_user on numeros (ID_Utilisateur, issuecode);
create index numeros_short_issuecode_index on numeros (short_issuecode);
create index publication_user on numeros (publicationcode, ID_Utilisateur);
create definer = root @`%` trigger numeros_issuecode_insert before
insert on numeros for each row BEGIN IF NEW.issuecode IS NULL
    AND EXISTS(
        ...
        where inducks_issue.publicationcode = NEW.publicationcode
            and inducks_issue.issuenumber = NEW.Numero
    ) THEN
set NEW.issuecode = (
...
create table numeros_popularite (
    ...
    constraint numeros_popularite_unique unique (Pays, Magazine, Numero)
) engine = MyISAM collate = utf8mb3_general_ci;
create index numeros_popularite_issuecode_unique on numeros_popularite (issuecode);
create table tranches_doublons (
    ...
    constraint tranches_doublons_Pays_Magazine_Numero_uindex unique (Pays, Magazine, Numero)
) engine = MyISAM collate = latin1_german2_ci;
create index tranches_doublons_tranches_pretes_ID_fk on tranches_doublons (TrancheReference);
create table tranches_pretes (
    ...
    slug varchar(30) as (
        concat(
            'edges-',
            replace(`publicationcode`, '/', '-'),
            '-',
            `issuenumber`
        )
    ),
    short_issuecode varchar(23) as (concat(`publicationcode`, ' ', `issuenumber`)),
    issuecode varchar(25) collate utf8mb3_unicode_ci not null,
    constraint tranches_pretes_short_issuecode_uindex unique (short_issuecode),
...
create table tranches_pretes_sprites (
    ...
    constraint tranches_pretes_sprites_unique unique (ID_Tranche, Sprite_name)
) engine = MyISAM charset = latin1;
...
create table tranches_pretes_sprites_urls (
    ...
    constraint tranches_pretes_sprites_urls_unique unique (Sprite_name, Version)
) charset = latin1;
create table users (
    ID int auto_increment primary key,
...
create table abonnements (
    ...
    constraint abonnements_unique unique (
        Pays,
        Magazine,
        ID_Utilisateur,
        Date_debut,
    ...
    constraint abonnements_unique2 unique (
        publicationcode,
        ID_Utilisateur,
        Date_debut,
        Date_fin
    ),
    constraint abonnements_users_ID_fk foreign key (ID_Utilisateur) references users (ID) on update cascade on delete cascade
) charset = latin1;
...
create table numeros_demandes (
    ...
    constraint numeros_demandes_unique unique (ID_Numero, ID_Acheteur),
    constraint numeros_demandes_users_null_fk foreign key (ID_Acheteur) references users (ID)
);
create table users_contributions (
    ID int auto_increment primary key,
    ID_user int not null,
    date datetime default current_timestamp() not null,
    contribution enum ('photographe', 'createur', 'duckhunter') collate utf8mb4_unicode_ci not null,
    points_new int not null,
    points_total int not null,
...
create index IDX_7FDC16F375567043 on users_contributions (ID_tranche);
create index IDX_7FDC16F3A5778B6C on users_contributions (ID_bookstore);
create index users_contributions__user_contribution on users_contributions (ID_user, contribution);
create table users_options (
    ...
    constraint users_options__unique unique (ID_User, Option_nom, Option_valeur),
    constraint users_options_users_ID_fk foreign key (ID_User) references users (ID)
) collate = latin1_german2_ci;
create index users_options__user_option on users_options (ID_User, Option_nom);
create table users_password_tokens (
    ID int auto_increment primary key,
    ID_User int not null,
    Token varchar(256) not null,
    constraint users_password_tokens_unique unique (ID_User, Token)
);
create table users_permissions (
    ...
    constraint permission_username_role unique (username, role, privilege)
) engine = MyISAM collate = latin1_german2_ci;
...
create table users_suggestions_notifications (
    ...
    constraint users_notifications__index_user_issue unique (ID_User, issuecode)
) charset = latin1;
create definer = root @`%` procedure generate_sprite_names() BEGIN TRUNCATE tranches_pretes_sprites;
INSERT INTO tranches_pretes_sprites(ID_Tranche, Sprite_name, Sprite_size)
SELECT tp.ID,
    get_sprite_name(tp.publicationcode, 'full'),
    (
        SELECT COUNT(*)
        FROM tranches_pretes tp2
        where tp2.publicationcode = tp.publicationcode
    )
...
SELECT tp.ID,
    get_sprite_name(
        tp.publicationcode,
        get_sprite_range(tp.issuenumber, 10)
    ),
    10
from tranches_pretes tp
...
SELECT tp.ID,
    get_sprite_name(
        tp.publicationcode,
        get_sprite_range(tp.issuenumber, 20)
    ),
    20
from tranches_pretes tp
WHERE tp.issuenumber regexp '^[0-9]+$';
...
SELECT tp.ID,
    get_sprite_name(
        tp.publicationcode,
        get_sprite_range(tp.issuenumber, 50)
    ),
    50
from tranches_pretes tp
WHERE tp.issuenumber regexp '^[0-9]+$';
...
SELECT tp.ID,
    get_sprite_name(
        tp.publicationcode,
        get_sprite_range(tp.issuenumber, 100)
    ),
    100
from tranches_pretes tp
WHERE tp.issuenumber regexp '^[0-9]+$';
END;
create definer = root @localhost function get_sprite_name(publicationcode varchar(12), suffix varchar(30)) returns varchar(48) RETURN concat(
    'edges-',
    REPLACE(publicationcode, '/', '-'),
    '-',
    suffix
);
create definer = root @localhost function get_sprite_range(issuenumber varchar(10), rangewidth int(10)) returns varchar(30) RETURN concat(
    issuenumber - mod(issuenumber - 1, rangewidth),
    '-',
    issuenumber - mod(issuenumber - 1, rangewidth) + rangewidth - 1
);
create definer = root @`%` procedure reset_issue_popularities() BEGIN
UPDATE numeros n
    INNER JOIN (
...
SET points = (
        ...
        WHERE np.Pays = SUBSTRING(
                tp.publicationcode,
                1,
                POSITION('/' IN tp.publicationcode) - 1
            )
            AND np.Magazine = SUBSTRING(
                tp.publicationcode,
                POSITION('/' IN tp.publicationcode) + 1
            )
