...
(
    ...
    issuecode       varchar(25) default concat(convert(`Pays` using utf8mb3), '/', convert(`Magazine` using utf8mb3),
                                               ' ', convert(`Numero` using utf8mb3)) not null,
    primary key (Pays, Magazine, Numero)
)
    charset = latin1;

create index abonnements_sorties_issuecode_unique
    on abonnements_sorties (issuecode);
...
(
    ...
    publicationcode varchar(14)                                  default concat(convert(`Pays` using utf8mb3), '/',
                                                                                convert(`Magazine` using utf8mb3)) not null,
...
create index issuecode_creationdate
    on numeros (DateAjout);

create index issuecode_user
    on numeros (ID_Utilisateur, issuecode);

create index numeros_short_issuecode_index
    on numeros (short_issuecode);

create index publication_user
    on numeros (publicationcode, ID_Utilisateur);
...
create index numeros_popularite_issuecode_unique
    on numeros_popularite (issuecode);
...
(
    ...
    constraint abonnements_unique2
        unique (publicationcode, ID_Utilisateur, Date_debut, Date_fin),
...
(
    ...
    ID_Utilisateur int         null,
    constraint etiquettes_unique
        unique (Description, ID_Utilisateur),
    constraint etiquettes_users_ID_fk
        foreign key (ID_Utilisateur) references users (ID)
            on delete cascade
);
...
create table numeros_etiquettes
(
    id           int auto_increment
        primary key,
    ID_etiquette int not null,
    ID_numero    int not null,
    constraint numeros_etiquettes_etiquettes_ID_fk
        foreign key (ID_etiquette) references etiquettes (ID)
            on delete cascade,
    constraint numeros_etiquettes_numeros_ID_fk
        foreign key (ID_numero) references numeros (ID)
            on delete cascade
);

create index numeros_etiquettes_ID_Etiquette_index
    on numeros_etiquettes (ID_etiquette);

create index numeros_etiquettes_ID_numero_index
    on numeros_etiquettes (ID_numero);
...
BEGIN
    ...
    UPDATE numeros n
        INNER JOIN (SELECT DISTINCT n_inner.Pays,
                                    n_inner.Magazine,
                                    n_inner.Numero
                    ...
                    WHERE n_inner.NUMERO NOT REGEXP '^[0-9]+$'
                      ...
                      AND n_inner.Numero != n2_inner.Numero) n2
    ...
    SELECT DISTINCT n.Pays,
                    ...
                    COUNT(*) AS Popularite
    FROM numeros n
    WHERE n.ID_Utilisateur NOT IN (SELECT u.ID
                                   FROM users u
    ...
    SET points = (SELECT GREATEST(1, Popularite)
