...
create index index_intervalles on edgecreator_intervalles (ID_Valeur, Numero_debut, Numero_fin, username);
...
create index edgecreator_valeurs_edgecreator_modeles2_ID_fk on edgecreator_valeurs (ID_Option);
...
create table images_tranches (
    ...
    constraint images_tranches_Hash_uindex unique (Hash)
);
create table tranches_en_cours_modeles (
    ...
    issuecode varchar(21) default concat(
    ...
    constraint tranches_en_cours_modeles__issuecode unique (issuecode, username),
    constraint tranches_en_cours_modeles__numero unique (Pays, Magazine, Numero, username)
);
create table tranches_en_cours_contributeurs (
    ...
    constraint tranches_en_cours_contributeurs__unique unique (ID_Modele, ID_Utilisateur, contribution),
    constraint tranches_en_cours_contributeurs_tranches_en_cours_modeles_ID_fk foreign key (ID_Modele) references tranches_en_cours_modeles (ID)
) charset = latin1;
create definer = root @`%` trigger tranches_en_cours_modeles_issuecode_insert before
...
create table tranches_en_cours_modeles_images (
    ...
    constraint tranches_en_cours_modeles_images___fk_image foreign key (ID_Image) references images_tranches (ID),
    constraint tranches_en_cours_modeles_images___modele foreign key (ID_Modele) references tranches_en_cours_modeles (ID)
) charset = latin1;
create table tranches_en_cours_valeurs (
    ...
    Option_valeur varchar(200) null,
    ID_Modele int null,
    constraint FK_AC57D99E4A1ED576 foreign key (ID_Modele) references tranches_en_cours_modeles (ID)
);
create index ID_Modele on tranches_en_cours_valeurs (ID_Modele);
create table users (
    ID int auto_increment primary key,
    username varchar(25) not null,
...
from (
        `edgecreator`.`tranches_en_cours_modeles`
...
order by `edgecreator`.`tranches_en_cours_modeles`.`Pays`,
    `edgecreator`.`tranches_en_cours_modeles`.`Magazine`,
    `edgecreator`.`tranches_en_cours_valeurs`.`Ordre`,
    `edgecreator`.`tranches_en_cours_valeurs`.`Nom_fonction`,
