...
docker exec -it ${coverid_container} /bin/bash -c "mysql -uroot -p${MYSQL_COVER_INFO_PASSWORD} -h ${MYSQL_COVER_INFO_HOST} ${MYSQL_COVER_INFO_DATABASE} < /tmp/${backup_source_filename_unzipped}"
