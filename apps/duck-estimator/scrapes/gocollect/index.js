const { createQuotations } = require('../../coa')
...
module.exports = {
  async scrape () {
    ...
    await readCsvMapping(MAPPING_FILE, record => mappedPublications.push(record))
      ...
      while (true) {
        ...
        for (const issueLinkHref of issueLinks) {
          ...
          await syncScrapeCache(
            ...
            cacheFileName,
            issueLinkHref,
            async (url) => await issuePage.goto(url),
            contents => issuePage.setContent(contents.toString()),
            async _contents => {
