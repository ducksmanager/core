...
db.connect().then(async () => {
  ...
  for (const dbName2 of [dbName, `${dbName}_new`]) {
    await db.runQuery(
      `USE ${dbName2}; ${readFileSync("sql/ddl/create-stats-tables.sql").toString()}`,
    );
  }

  await db.runQuery(
    readFileSync("sql/create-stories-publications.sql").toString(),
  );
  await db.runQuery(readFileSync("sql/create-authors-stories.sql").toString());
  await db.runQuery(
    readFileSync("sql/create-users-missing-stories.sql").toString(),
  );
  await db.runQuery(
    readFileSync("sql/create-users-missing-issues.sql").toString(),
  );
  await db.runQuery(
    readFileSync("sql/create-users-suggested-issues.sql").toString(),
  );
