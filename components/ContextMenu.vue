<template>
  <v-contextmenu-group>
    ...
    <b-alert
      ...
      variant="danger"
      >{{
        $t("Vous allez retirer tous les exemplaires des numéros sélectionnés")
      }}></b-alert
    >
    <b-alert
      ...
      >{{
        $t(
          "Vous possédez certains numéros sélectionnés en plusieurs exemplaires. Seul le premier exemplaire sera modifié."
        )
      }}</b-alert
    >
    <b-alert
      ...
      >{{
        $t(
          "Sélectionnez un ou plusieurs numéros dans la liste pour les ajouter, modifier ou supprimer de votre collection."
        )
      }}
    </b-alert>
    <template v-else>
          ...
          <v-contextmenu-group :title="$t('Date d\'achat')">
            ...
            >
              <v-contextmenu-item
                ...
                :key="`copy-${copyIndex}-purchase-item-${purchaseStateId}`"
                :hide-on-click="false"
                :class="{
                  clickable: true,
              ...
              <v-contextmenu-submenu
                v-else
                :key="`copy-${copyIndex}-purchase-submenu-${purchaseStateId}`"
...
const isSingleIssueSelected = $computed(
  () => props.selectedIssues.length === 1
);
