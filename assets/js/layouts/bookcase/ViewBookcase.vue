<template>
  <div>
    <div v-if="!isSharedBookcase">
      ...
      <div
        v-if="
          user?.isShareEnabled && username !== 'demo' && sortedBookcase?.length
        "
        class="mb-4"
      >
        <b-alert
      ...
      <b-alert
        v-else-if="user?.isShareEnabled === false && username !== 'demo'"
        show
        variant="warning"
        v-html="
          $t(
...
export default {
  ...
  computed: {
    ...
    loading() {
      return (
        !this.isPrivateBookcase &&
        !this.isUserNotExisting &&
        !(this.sortedBookcase && this.bookcaseOptions && this.edgesUsingSprites)
      );
    },
    ...
    percentVisible() {
      return !this.bookcase?.length
        ? null
        : parseInt(
            (100 * this.bookcase.filter(({ edgeId }) => edgeId).length) /
              this.bookcase.length
          );
    },

    mostPopularIssuesInCollectionWithoutEdge() {
      return [...this.popularIssuesInCollectionWithoutEdge]
        ?.sort(
        ...
        [...this.bookcase].sort(
          (
            ...
            return (
              ...
              (!vm.issueNumbers[publicationCode1] && -1) ||
              Math.sign(
                vm.issueNumbers[publicationCode1].indexOf(
            ...
            newValue
              ...
              .reduce(
                (acc, publicationCode) => ({
                  ...acc,
                  ...{
                    [publicationCode]: vm.bookcase.filter(
      ...
      async handler(newValue) {
        if (newValue) {
          ...
          this.edgesUsingSprites = usableSprites
            ...
            .sort(({ size: aSize }, { size: bSize }) =>
              Math.sign(aSize - bSize)
            )
  ...
  methods: {
    ...
    highlightIssue(issue) {
      this.currentEdgeHighlighted = this.bookcase.find(
        (issueInCollection) =>
