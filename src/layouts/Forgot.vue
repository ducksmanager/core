<template>
  <div>
    <b-alert v-if="isSuccess" show variant="info"
      ><template v-if="token">
        {{
          $t(
    ...
    <form v-else method="post">
      <b-alert v-if="isSuccess === false" show variant="danger"
        ><template v-if="token">{{ $t(`Une erreur s'est produite.`) }}</template
        ><template v-else
          >$t( 'Le champ E-mail est invalide, correspond-il à un email
          enregistré sur DucksManager ?' )
        </template></b-alert
      >
      <template
        v-if="
...
const { errors, success, token } = defineProps({
